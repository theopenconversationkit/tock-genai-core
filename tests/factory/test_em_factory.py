...
@pytest.mark.parametrize(
    ...
    [
        (BloomZEMSetting(provider=EMProvider.BloomZ, api_base="http://api.com"), BloomzFactory),
        (
            AzureOpenAIEMSetting(
                provider=EMProvider.AzureOpenAI, api_base="http://api.com", api_version="1.0.0", deployment="deployment"
            ),
            AzureOpenAIEMFactory,
        ),
        (VLLMEMSetting(provider=EMProvider.Vllm, api_base="http://api.com", model="model"), VLLMEMFactory),
    ],
)
