...
from tock_genai_core.models.llm import LLMProvider, AzureOpenAILLMSetting, HuggingFaceTextGenInferenceLLMSetting, VllmSetting
...
@pytest.mark.parametrize("settings,  expected_output", [
    (HuggingFaceTextGenInferenceLLMSetting(provider= LLMProvider.TGI, model="model", temperature=0.5, repetition_penalty=1.0, max_new_tokens=256, api_base="http://api.com", streaming=False), 
     TGIFactory),
    (AzureOpenAILLMSetting(provider=LLMProvider.AzureOpenAI, model="model", temperature=0.5, api_base="http://api.com", api_version="1.0.0", deployment="deployment"), 
